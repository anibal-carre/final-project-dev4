...
export function ModalProfile({ id }) {
  ...
  const idRol = userData.usuario.idrol;

  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/usuarios/${userData.usuario.id}`)
      .then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/api/usuarios/${userData.usuario.id}`,
        {
          usuario,
          clave,
          idRol,
